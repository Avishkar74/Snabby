...
## ADR 22: Composited Rendered Image as Single Source of Truth for OCR & Race-Condition Safety

### Status
Accepted & Implemented

### Context
When users edit a page, they can draw vector shapes, write text, and insert external image files from their local computer. These added elements frequently contain text (e.g. diagrams, charts, pasted snippets). Previously:
1. Heuristic attempts to extract text from Excalidraw JSON nodes lacked spatial accuracy, failed to align with canvas transformations, and completely ignored text contained inside added local images.
2. OCR results were tied to original screenshots, leaving edited pages with stale selectable text overlays or out-of-sync PDF text layers.
3. Rapid consecutive saves risked race conditions where an older OCR job completing late could overwrite newer OCR data.

### Decisions

1. **Composited Rendered Image as Single OCR Source**:
   The final composited visual image (`page.effectiveRenderedImageId`) generated by `renderBoundedPageImage` contains all layers: original screenshot + vector drawings + canvas text + added local images. Running OCR on this exact flattened image guarantees universal text recognition with pixel-accurate word bounding boxes regardless of whether the text originated from the webpage screenshot, an Excalidraw text element, or an added local PNG/JPEG.

2. **Strict Freshness Invariant**:
   An OCR result is valid if and only if:
   $$\text{ocrResult.processedImageId} === \text{page.effectiveRenderedImageId}$$
   Stale OCR is strictly rejected for Lightbox Preview text selection overlays, PDF text layers, and thumbnail completion indicators.

3. **Non-Blocking Asynchronous Save Lifecycle**:
   Saving annotations in `SAVE_PAGE_ANNOTATIONS` must remain instantaneous. The new composited `ImageAsset` is stored, `page.renderedImageId` is updated, and the message handler immediately returns `{ success: true }` to the UI before kicking off OCR asynchronously in the background.

4. **Single-Flight Deduplication**:
   `RunOCR` deduplicates concurrent requests for the same `(pageId, imageId)` using an internal `inFlightJobs` map. Subsequent requests (such as simultaneous auto-healing and save callbacks) share the in-flight Promise rather than launching duplicate Tesseract workers.

5. **Persistence-Time Freshness Validation**:
   Before writing an OCR result to `OCRRepository` or marking `Page.status = COMPLETED`, `RunOCR` re-reads the latest `Page` from `PageRepository`. If the page's `effectiveRenderedImageId` changed while OCR was in flight (e.g. rapid consecutive saves A -> B), the outdated OCR result for image A is discarded, avoiding state corruption.

6. **Non-Blocking Auto-Healing in Preview**:
   `GET_PAGE_OCR` returns `{ success: true, data: { ocrResult: null, currentRenderedImageId } }` immediately when OCR is missing or stale, while triggering background OCR. The Lightbox Preview displays the image immediately with no overlay, refreshing smoothly once `OCR_COMPLETED` is broadcast.

7. **Zero-Retry PDF Generation**:
   `PdfLibPDFService` strictly embeds text layers only when fresh OCR is present. If OCR is missing or stale, it renders the visual image cleanly without attempting inline OCR retries.
