...
✓ Lightbox preview with keyboard navigation (Arrows / Esc / E), prominent Edit button, strict OCR freshness enforcement, and auto-refresh on OCR_COMPLETED
✓ Selectable OCR overlay with transparent text, native selection highlights, and ResizeObserver auto-alignment
✓ Capture deletion
...
✓ Non-blocking annotation saving with asynchronous background OCR
...
| **Main App Container** | `src/app/App.tsx` | Hosts mascot SVG logo, coordinates active/new session view switching, lightbox, PageEditor modal, and toast container. Synchronizes `selectedCapture` with updated session captures. | `useSession`, `useCaptures`, `usePdfExporter`, `MessageBusContext` |
...
| **Captures Hook** | `src/features/capture/hooks/useCaptures.ts` | Queries `GET_ALL_THUMBNAILS`, manages thumbnail list with `effectiveRenderedImageId`, sends `DELETE_CAPTURE`, listens for `CAPTURE_COMPLETE`, `OCR_COMPLETED`, `OCR_FAILED`, `SESSION_UPDATED`. | `useMessageBus` |
...
| **Lightbox Preview** | `src/features/capture/components/LightboxPreview.tsx` | Full-screen image lightbox modal with prominent blue Edit button in header control group, backdrop dismiss, and keyboard navigation (Arrows / Esc / E). Queries `GET_PAGE_OCR`, validates OCR freshness against `activeRenderedImageId`, auto-refreshes on `OCR_COMPLETED` and `SESSION_UPDATED`, and measures rendered image bounds via `ResizeObserver`. | React, DOM keyboard events, `useMessageBus` |
...
| **Page Editor Modal** | `src/features/page-editor/components/PageEditor.tsx` | Full-screen modal overlay hosting Excalidraw canvas. Performs scene initialization (`useMemo`), debounced auto-save, overlay event containment, and ESC shortcut. On save, triggers non-blocking image persistence and background OCR. | Excalidraw, `renderBoundedPageImage`, `useMessageBus` |
